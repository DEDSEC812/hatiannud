...
import { useConfirmAge, getGetMeQueryKey } from "@workspace/api-client-react";
...
export function AgeGate() {
  ...
  const handleConfirm = async () => {
    ...
    if (isSignedIn) {
      try {
        await confirmAgeMutation.mutateAsync(undefined as never);
        queryClient.invalidateQueries({ queryKey: getGetMeQueryKey() });
      } catch (e) {
        console.error("Failed to sync age confirmation to profile", e);
      }
    }
  };
    ...
    <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-md border-border bg-card text-card-foreground shadow-2xl [&>button]:hidden">
        ...
        <DialogFooter className="flex-col sm:flex-row gap-3 sm:gap-2 mt-6">
          ...
          <Button className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold" onClick={handleConfirm} disabled={confirmAgeMutation.isPending}>
            J'ai 18 ans ou plus
          </Button>
        </DialogFooter>
      </DialogContent>
