...
import { useListVideos, useTrendingVideos, type Video as VideoType } from "@workspace/api-client-react";
import { VideoCard } from "@/components/video-card";
import { Button } from "@/components/ui/button";
import { Link } from "wouter";
import { Play, TrendingUp, Star, ChevronRight, Home as HomeIcon, Video, Image as ImageIcon, Flame, Download } from "lucide-react";
...
export function Home() {
  ...
  const { data: trending, isLoading: isLoadingTrending } = useTrendingVideos();
  const { data: latest, isLoading: isLoadingLatest } = useListVideos({ });
  const basePath = import.meta.env.BASE_URL.replace(/\/$/, "");

  const visibleVideos = (() => {
    if (!latest) return latest;
    if (activeTab === "popular") return [...latest].sort((a, b) => b.views - a.views);
    if (activeTab === "downloads") return latest.filter((v: VideoType) => !v.isVip);
    return latest;
  })();
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {isLoadingTrending ? (
              ...
              trending.slice(0, 4).map((video: VideoType, i: number) => (
          ...
          {isPhotoTab ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 gap-y-10">
              {isLoadingLatest ? (
                ...
                visibleVideos.map((video: VideoType, i: number) => (
