...
import { useGetVideo, useRegisterView, useRequestDownload, useGetAdsConfig, useListComments, useCreateComment, useListVideos, getListCommentsQueryKey, type Video as VideoType, type Comment } from "@workspace/api-client-react";
...
import { Download, ThumbsUp, Share2, Star, MessageSquare, Play, AlertCircle, Send, Lock, Link as LinkIcon, Copy } from "lucide-react";
...
export function Watch() {
  ...
  const { data: video, isLoading: isLoadingVideo } = useGetVideo(id!);
  ...
  const { data: relatedVideos, isLoading: isLoadingRelated } = useListVideos({ category: video?.category });
  ...
  useEffect(() => {
    if (id) {
      registerViewMutation.mutate({ id });
    }
  }, [id]);
          ...
          <div className="mb-6">
            <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {!video.isVip && (
                  <Button onClick={handleDownload} disabled={requestDownloadMutation.isPending} className="rounded-full bg-primary hover:bg-primary/90 text-white shadow-[0_0_15px_rgba(30,94,255,0.3)]">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Button type="submit" disabled={!commentBody.trim() || createCommentMutation.isPending}>Commenter</Button>
                    </div>
                  </div>
                </div>
              </form>
              ...
              {isLoadingComments ? (
                ...
                comments.map((comment: Comment) => (
                  <div key={comment.id} className="flex gap-4">
                    <Avatar className="h-10 w-10 shrink-0">
                      <AvatarFallback className="bg-primary/10 text-primary">{comment.displayName.charAt(0).toUpperCase()}</AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="flex items-baseline gap-2 mb-1">
                        <span className="font-semibold text-sm">{comment.anonymous ? "Anonyme" : comment.displayName}</span>
        ...
        <div className="lg:col-span-1">
          ...
          <div className="flex flex-col gap-4">
            {isLoadingRelated ? (
              ...
              relatedVideos.filter((v: VideoType) => v.id !== id).map((vid: VideoType) => (
